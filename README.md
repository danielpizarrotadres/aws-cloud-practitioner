...
But it should probably also be able to send requests to the internet itself, for example to dowload the web server software and install it when the instance starts up.
    ...
    - However, the instance that is running the database probably should not accept incoming requests from the internet (No inbound traffic should be allowed) becase it should only talk to the web server instance, not to some random person from the internet. No one should be able to send direct requests to our application database. But the instance should still be able to send requests itself. For example, to dowload patches for the database software.
    
    - And this is not a super complex setup, but it is a quite a common setup, which can only be implemented with AWS with help of this VPC service. Because to build this kind of setup, you want to be able to have detailed control over which instances are connected to the internet in which way, and which instances should not be connected, or maybe not connected such that requests can be sent. And in the previous course section you learned that when you start an instance, you can actually control which kind of requests are allowed. That gives you a great amount of security. However, what you are configuring there is a so-called security group. It is essentially though a firewall for the instance you are launching. And with that firewall, you simply control or configure that certain incoming requests can be blocked. That is greats and you should that.
